...
### Что реализовано
- **UI**: Material 3, тёмная/светлая тема, AnimatedSwitcher(FADE), адаптивный сайдбар
- **Транзакции**: доход/расход, группировка по дате, фильтр, поиск, CSV export/import
- **Отчёты**: кастомные столбчатые графики расходов по категориям и по дням
- **Обзор**: двойной столбчатый график динамики доходов/расходов (независимые Y-шкалы)
- **Цели**: CRUD, кольцевой прогресс-индикатор, атомарное пополнение, сводный баннер
- **Напоминания**: CRUD, повторяемость, подсветка просроченных/ближайших
- **Категории**: CRUD с выбором иконки (20 вариантов) и цвета (10 вариантов)
- **Безопасность**: БД только в памяти, AES-256-GCM, атомарная запись на диск
- **Качество**: ruff + mypy + pytest (55 тестов) + pip-audit, CI на Linux и Windows
...
> - `requirements/app.txt` — полный набор для desktop (flet, flet-charts, cryptography).
...
`mypy` запускается на `src/core` и `src/infra` (см. `pyproject.toml`). UI-слой
(`src/ui_flet`, `main_flet.py`) пока вне strict-проверки — там ~70 типовых
рассогласований с API Flet, это отдельная задача.

### Запуск приложения (desktop)

Material 3 тема с плавными переходами. При первом запуске создаётся пароль — БД
хранится только в памяти и записывается на диск в зашифрованном виде.
...
Web-режим (доступен с любого устройства в локальной сети):

```powershell
py -3.12 main_flet.py --web
# затем открыть http://<IP-ПК>:8550 в браузере
```
...
### Логи
Логи пишутся в `data/logs/app.log` (рядом с зашифрованной БД), с ротацией
1 МБ × 5 файлов. Уровень по умолчанию — `INFO`; меняется через переменную
окружения `PF_LOG_LEVEL=DEBUG|INFO|WARNING|ERROR`. Предупреждения и ошибки
дублируются в stderr.
...
- **БД работает только в памяти**: после ввода пароля зашифрованный файл расшифровывается
  напрямую в `:memory:`-соединение SQLite. Plaintext **никогда не пишется на диск** —
  ни в `%TEMP%`, ни рядом с приложением. Снимки сохраняются обратно в `*.enc` после
  каждого commit'а и при закрытии (атомарная запись через `*.tmp` + rename).
- **Аварийное завершение**: если процесс убит между коммитами — данные с последнего
  успешного коммита сохранены; всё, что было после, теряется. Plaintext на диске
  при этом не остаётся.
- **Legacy-очистка**: при старте удаляются любые `%TEMP%/pfm_*.sqlite3` от старых
  версий приложения, в которых использовался файловый plaintext-режим.
- **Модель угроз (упрощённо)**: защита от чтения данных при копировании файла БД;
  не покрывает компрометацию ОС и дамп памяти процесса.
